...
typedef struct {
    ...
    uint8_t data[256];                  // Fixed size data array
} FuzzInstruction;
...
typedef struct {
    ...
    FuzzInstruction instructions[32];   // Instruction array
} FuzzVariant;
...
typedef struct {
    ...
    FuzzInstruction instructions[32];   // Instruction array (up to 32)
    
    // ✅ Batch variant mode
    FuzzVariant variants[10];           // Up to 10 variants
} FuzzSharedMemory;
