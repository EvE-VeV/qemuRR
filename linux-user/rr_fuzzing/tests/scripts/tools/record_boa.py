...
TRACE_FILE = "/home/webfuzz/Documents/qemu/linux-user/rr_fuzzing/traces/boa_totolink.trace"
BOA_CMD = ["bin/boa", "-c", ".", "-d"]
...
def run_recording():
    ...
    try:
        # 1. Login attempt (Basic Auth) - sets state inside boa (hopefully)
        requests.get("http://127.0.0.1:8080/boafrm/formLogin", auth=("admin", "admin"))
        ...
        # Use a session to keep connection alive if possible
        s = requests.Session()
        s.auth = ("admin", "admin")
        r = s.post("http://127.0.0.1:8080/boafrm/formPortFw", data=data)
